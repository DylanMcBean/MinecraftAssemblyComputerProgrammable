...
scoreboard players operation Cache_B computer = Cache_B Caches
#Logging Info
execute if score Basic Debugging > Reg_Zero computer run tellraw @p ["",{"text":"Read ","extra":[{"score":{"name":"Cache_B","objective":"computer"}, "color":"red"}]},{"text":" from Address ","extra":[{"score":{"name":"Address","objective":"computer"},"color":"red"}]},{"text":" into Registry B"}]